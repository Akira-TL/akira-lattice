...
全局 `core/AGENTS.md` 只保留“什么时候考虑多 Agent、默认模型和写入隔离”等长期策略；具体编排步骤由本 Skill 按需加载。

DevSpace 的 `workspaceId`、linked worktree 恢复方式等工具事实放在 `core/references/devspace.md`，避免把 workflow 和 reference 混为一体。
