...
- 收紧机器级 Skill 注册表与项目级 Skill view 的边界：当前会话缺少 Skill 时仍先检查 `~/.agents/skills/`，机器级已安装只表示可复用能力，不再自动推导为创建项目软链接。`<project>/.agents/skills/` 继续作为开放 Agent Skills 生态允许的显式项目级 view；只有项目确实需要时，Agent 才可主动建立指向机器级注册项的软链接。机器级注册表不会自动投影到项目目录；项目中已显式存在的 `.agents/skills/` 按项目约定正常使用。
