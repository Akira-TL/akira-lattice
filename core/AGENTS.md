...
- 任务需要某个 Skill 而当前会话不可用时，先检查机器级注册表 `~/.agents/skills/<skill>`。若已存在，不重复安装；当前执行器若需要自己的 Skill 目录，由执行器自己的机制引用该机器级 Skill。只有机器级也不存在时，才使用 `akira` Skill Router 确认来源、用途与最小安装集合；取得用户明确同意后，由 `akira` Skill 自带安装脚本完成远端 source 与机器级注册。Lattice 根安装器不参与 Skill 生命周期。
