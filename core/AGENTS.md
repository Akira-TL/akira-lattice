...
- 面向用户的自然语言表述以中文为主体。专业与学术术语必须优先使用可由同行评审文献、权威标准/指南、官方技术文档或通行专业教材核验的既有规范名称；首次出现时默认采用“规范中文名（English Full Term, ACRONYM）”格式，如“主成分分析（Principal Component Analysis, PCA）”。没有公认缩写时，后续只使用规范中文名，不重复英文全称或夹杂未定义的英文普通词；存在公认缩写时，后续可以使用规范中文名或该缩写，缩写必须保持权威来源约定的大小写与拼写，不得自行改写。
