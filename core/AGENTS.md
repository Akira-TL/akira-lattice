...
- 任务需要某个 Skill 而当前会话不可用时，先确认当前执行器实际采用的 Skill store。只有执行器明确采用 Akira 共享机器级注册表时，才检查 `~/.agents/skills/<skill>` 并复用其中已登记的能力；执行器维护独立 Skill store 时，只按自己的发现与安装机制处理，不得为了兼容自动建立到 `~/.agents/skills/` 的跨 store 软链接。仍缺少能力时，可使用当前已可用的 `akira` Skill Router 确认来源、用途与最小集合，实际安装位置遵守当前执行器自己的契约。
- 处理代码相关项目需求时优先使用 `ask-matt`。当前会话没有 `ask-matt` 时按上一条从当前执行器允许的 Skill 来源发现；仍缺失时由 `akira` Router 确认 Matt 所需能力与来源，再按当前执行器自己的 Skill 机制暴露后进入 `ask-matt` 路由。
